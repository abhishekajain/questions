...
public class StockTrade {
	...
	public static int getMaxProfitBruftForce(int[] stockPrices) {
		...
		int result = stockPrices[1] - stockPrices[0];		
		int start = 0;
		while(start<len) {
			int count = start+1;
			while(count<len) {
				int temp = stockPrices[count] - stockPrices[start];
				if(temp>result) {
					result = temp;
				}
				count++;
	...
	public static int getMaxProfit(int[] stockPrices) {
		...
		int minPrize = stockPrices[0];
		int maxProfit = stockPrices[1] - stockPrices[0];
		// calculate the max profit
		int i = 1;
		while(i<stockPrices.length){
			int currentPrize = stockPrices[i];
			int profit = currentPrize - minPrize;
			maxProfit = 	Math.max(profit, maxProfit);
			minPrize = Math.min(currentPrize, minPrize);
			i++;
		}
		return maxProfit;
	}
	...
	public static void main(String[] args) {
		Result result = JUnitCore.runClasses(StockTrade.class);
